...
// Global variable to control rotation
...
void updateAnimationLoop() {
	...
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
		...
		curr_angleZ -= 0.005;
		curr_angle = curr_angleZ;
		curr_dir = glm::vec3(0.0f, 0.0f, 1.0f);
	}
...
bool initializeMVPTransformation()
{
	...
	Model = glm::rotate(Model, curr_angle, curr_dir);
...
bool initializeVertexbuffer()
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, normalbuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(colors) * 6, &normals[0], GL_STATIC_DRAW);
